...
class SequenceLabelingModel(object):
    ...
    def build_model(self):
	    ...
	    else tf.concat(2,values=self.embedding_features, name='input_features')
           # else tf.concat(values=self.embedding_features, axis=2, name='input_features')
	    
        # bi-lstm

        if self.rnn_unit == 'lstm':
            #fw_cell = rnn.LayerNormBasicLSTMCell(self.nb_hidden, forget_bias=1.)
            fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.nb_hidden, forget_bias=1., state_is_tuple=True)
            #bw_cell = rnn.LayerNormBasicLSTMCell(self.nb_hidden, forget_bias=1.)
            bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.nb_hidden, forget_bias=1., state_is_tuple=True)
        ...
        lstm_output = tf.nn.dropout(
            tf.concat(2,rnn_outputs, name='lstm_output'),
